...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents) + len(additional_contents) <= self.volume:
            ...
            for k in range(self.volume, len(self.contents) + len(additional_contents)):
                s = s + list(self.contents + additional_contents)[k]
            for i in range(len(self.contents), self.volume):
                self.contents = self.contents + list(temp + additional_contents)[i]
        return s
    ...
    def shrink(self, new_volume):
        ...
        for k in range(new_volume, len(self.contents)):
            excess = excess + list(self.contents)[k]
        for k in range(new_volume):
            new_contents = new_contents + list(self.contents)[k]
